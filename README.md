Here is the final `README.md` file. It is in English and ready to be copied.
...
## Key Features
  ...
  * **CSV Upload:** Accepts CSV file uploads and parses them using efficient streaming to handle large files.
  ...
  * **Status Styling:** Provides clear visual styling for `PENDING` (yellow) and `FAILED` (red) statuses.
...
### Backend (Go)
  ...
  * **Backend-Driven Pagination:** Instead of sending thousands of issues to the frontend, we implemented server-side pagination and sorting (`GET /issues?page=...`). This is scalable and keeps the frontend lightweight.
...
### DevOps (Extra Features)
  ...
  * **Multi-stage Dockerfile:** We use a multi-stage build for the `backend/Dockerfile` to ensure the final production image is minimal (containing only the Go binary) and secure.
  * **Monorepo CI:** We created two separate GitHub Actions workflows. Both are triggered by `paths` so that a change in `frontend/` does not run the `backend/` CI, saving resources.
      * **`backend.yml`:** Runs a Go linter (`golangci-lint`), vulnerability scanner (`govulncheck`), and unit tests (`go test -race`).
